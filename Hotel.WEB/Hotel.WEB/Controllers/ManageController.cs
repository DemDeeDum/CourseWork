    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            var model = new IndexViewModel
            {
                ...
                PhoneNumber = await manageService.GetPhoneNumberAsync(userId, UserManager),
                TwoFactor = await manageService.GetTwoFactorEnabledAsync(userId, UserManager),
                Logins = await manageService.GetLoginsAsync(userId, UserManager),
                BrowserRemembered = await AuthenticationManager.TwoFactorBrowserRememberedAsync(userId),
                ActiveBookings = profileService.GetActiveUserBooking(userId).
                Select(Mapper.Map<BookingDTO, BookingViewModel>).OrderBy(x => x.BegginingTime).ToList(),
                BookingsToConfirm = profileService.GetUserBookingToConfirm(userId).
                Select(Mapper.Map<BookingDTO, BookingViewModel>).OrderBy(x => x.BegginingTime).ToList(),
                PastBookings = profileService.GetPastUserBooking(userId).
                Select(Mapper.Map<BookingDTO, BookingViewModel>).OrderBy(x => x.BegginingTime).ToList()
            };
            return View(model);
        }
        ...
        public async Task<ActionResult> ManageLogins(ManageMessageId? message)
        {
            ...
            var userLogins = await manageService.GetLoginsAsync(User.Identity.GetUserId(), this.UserManager);
            var otherLogins = AuthenticationManager.GetExternalAuthenticationTypes().Where(auth => userLogins.All(ul => auth.AuthenticationType != ul.LoginProvider)).ToList();
            ViewBag.ShowRemoveButton = 
                await manageService.GetPasswordHash(User.Identity.GetUserId(), this.UserManager) != null || userLogins.Count > 1;
